...
1. In Synapse Studio, navigate to the **Manage** hub.
    ...
    ![Manage hub.](../data/images/manage-hub.png "Manage hub")

2. Select **Linked services** on the left, then select **+ New**.

    ![The New button is highlighted.](../data/images/linked-services-new.png "Linked services")

3. Select **Azure SQL Database**, then select **Continue**.

    ![Azure SQL Database is selected.](../data/images/new-linked-service-sql.png "New linked service")
...
4. Complete the new linked service form as follows:
    ...
    ![The form is completed as described.](../data/images/new-linked-service-sql-form.png "New linked service form")
...
1. Navigate to the **Develop** hub.
    ...
    ![Develop hub.](../data/images/develop-hub.png "Develop hub")

2. Select **+**, then select **Data flow**.

    ![The plus button and data flow menu item are highlighted.](../data/images/new-data-flow.png "New data flow")
...
3. In the properties pane of the new data flow, enter `UpdateCustomerDimension` in the **Name** field **(1)**, then select the **Properties** button **(2)** to hide the properties pane.
    ...
    ![The data flow properties pane is displayed.](../data/images/data-flow-properties.png "Properties")
...
4. Select **Data flow debug** to enable the debugger. This will allow us to preview data transformations and debug the data flow before executing it in a pipeline.
    ...
    ![The Data Flow debug button is highlighted.](../data/images/data-flow-turn-on-debug.png "Data flow debug")

5. Select **OK** in the dialog that appears to turn on the data flow debug.

    ![The OK button is highlighted.](../data/images/data-flow-turn-on-debug-dialog.png "Turn on data flow debug")
...
6. Select **Add Source** on the canvas.
    ...
    ![The Add Source button is highlighted on the data flow canvas.](../data/images/data-flow-add-source.png "Add Source")
...
7. Under `Source settings`, configure the following properties:
    ...
    ![The New button is highlighted next to Dataset.](../data/images/data-flow-source-new-dataset.png "Source settings")

8. In the new integration dataset dialog, select **Azure SQL Database**, then select **Continue**.

    ![Azure SQL Database and the Continue button are highlighted.](../data/images/data-flow-new-integration-dataset-sql.png "New integration dataset")
...
9. In the dataset properties, configure the following:
    ...
    ![The form is configured as described.](../data/images/data-flow-new-integration-dataset-sql-form.png "Set properties")
...
11. The `SourceCustomer` dataset should now appear and be selected as the dataset for the source settings.
    ...
    ![The new dataset is selected in the source settings.](../data/images/data-flow-source-dataset.png "Source settings: Dataset selected")

12. Select **+** to the right of the `SourceDB` source on the canvas, then select **Derived Column**.

    ![The plus button and derived column menu item are both highlighted.](../data/images/data-flow-new-derived-column.png "New Derived Column")
...
13. Under `Derived column's settings`, configure the following properties:
    ...
    ![The form is configured as described.](../data/images/data-flow-derived-column-settings.png "Derived column settings")

14. Click in the **Expression** text box, then select **Open expression builder**.

    ![The open expression builder link is highlighted.](../data/images/data-flow-derived-column-expression-builder-link.png "Open expression builder")
...
15. Select **Refresh** next to `Data preview` to preview the output of the `HashKey` column, which uses the `sha2` function you added. you should see that each hash value is unique.
    ...
    ![The data preview is displayed.](../data/images/data-flow-derived-column-expression-builder.png "Visual expression builder")
...
17. Select **Add Source** on the canvas underneath the `SourceDB` source. We need to add the `DimCustomer` table located in the Synapse dedicated SQL pool to use when comparing the existence of records and for comparing hashes.
    ...
    ![The Add Source button is highlighted on the canvas.](../data/images/data-flow-add-source-synapse.png "Add Source")
...
18. Under `Source settings`, configure the following properties:
    ...
    ![The New button is highlighted next to Dataset.](../data/images/data-flow-source-new-dataset2.png "Source settings")

19. In the new integration dataset dialog, select **Azure Synapse Analytics**, then select **Continue**.

    ![Azure Synapse Analytics and the Continue button are highlighted.](../data/images/data-flow-new-integration-dataset-synapse.png "New integration dataset")
...
20. In the dataset properties, configure the following:
    ...
    ![The form is configured as described and the refresh button is highlighted.](../data/images/data-flow-new-integration-dataset-synapse-refresh.png "Refresh")

21. In the **Value** field, enter `SQLPool01`, then select **OK**.

    ![The SQLPool01 parameter is highlighted.](../data/images/data-flow-new-integration-dataset-synapse-parameter.png "Please provide actual value of the parameters to list tables")
...
22. Select `dbo.DimCustomer` under **Table name**, select `From connection/store` under **Import schema**, then select **OK** to create the dataset.
    ...
    ![The form is completed as described.](../data/images/data-flow-new-integration-dataset-synapse-form.png "Table name selected")

23. The `DimCustomer` dataset should now appear and be selected as the dataset for the source settings.

    ![The new dataset is selected in the source settings.](../data/images/data-flow-source-dataset2.png "Source settings: Dataset selected")

24. Select **Open** next to the `DimCustomer` dataset that you added.

    ![The open button is highlighted next to the new dataset.](../data/images/data-flow-source-dataset2-open.png "Open dataset")

25. Enter `SQLPool01` in the **Value** field next to `DBName`.

    ![The value field is highlighted.](../data/images/dimcustomer-dataset.png "DimCustomer dataset")
...
26. Switch back to your data flow. *Do not* close the `DimCustomer` dataset. Select **+** to the right of the `CreateCustomerHash` derived column on the canvas, then select **Exists**.
    ...
    ![The plus button and exists menu item are both highlighted.](../data/images/data-flow-new-exists.png "New Exists")
...
27. Under `Exists settings`, configure the following properties:
    ...
    ![The form is configured as described.](../data/images/data-flow-exists-form.png "Exists settings")

28. Select **+** to the right of `Exists` on the canvas, then select **Lookup**.

    ![The plus button and lookup menu item are both highlighted.](../data/images/data-flow-new-lookup.png "New Lookup")
...
29. Under `Lookup settings`, configure the following properties:
    ...
    ![The form is configured as described.](../data/images/data-flow-lookup-form.png "Lookup settings")

30. Select **+** to the right of `LookupCustomerID` on the canvas, then select **Derived Column**.

    ![The plus button and derived column menu item are both highlighted.](../data/images/data-flow-new-derived-column2.png "New Derived Column")
...
31. Under `Derived column's settings`, configure the following properties:
    ...
    ![The form is configured as described.](../data/images/data-flow-derived-column-settings2.png "Derived column settings")
...
32. Select **+** to the right of the `SetDates` derived column step on the canvas, then select **Alter Row**.
    ...
    ![The plus button and alter row menu item are both highlighted.](../data/images/data-flow-new-alter-row.png "New Alter Row")
...
33. Under `Alter row settings`, configure the following properties:
    ...
    ![The form is configured as described.](../data/images/data-flow-alter-row-settings.png "Alter row settings")

34. Select **+** to the right of the `AllowUpserts` alter row step on the canvas, then select **Sink**.

    ![The plus button and sink menu item are both highlighted.](../data/images/data-flow-new-sink.png "New Sink")
...
35. Under `Sink`, configure the following properties:
    ...
    ![The form is configured as described.](../data/images/data-flow-sink-form.png "Sink form")
...
36. Select the **Settings** tab and configure the following properties:
    ...
    ![The sink settings are configured as described.](../data/images/data-flow-sink-settings.png "Sink settings")
...
37. Select the **Mapping** tab, then uncheck **Auto mapping**. Configure the input columns mapping as outlined below:
    ...
    ![Mapping settings are configured as described.](../data/images/data-flow-sink-mapping.png "Mapping")

38. The completed mapping flow should look like the following. Select **Publish all** to save your changes.

    ![The completed data flow is displayed and Publish all is highlighted.](../data/images/data-flow-publish-all.png "Completed data flow - Publish all")

39. Select **Publish**.

    ![The publish button is highlighted.](../data/images/publish-all.png "Publish all")
...
1. Navigate to the **Integrate** hub.
    ...
    ![Integrate hub.](../data/images/integrate-hub.png "Integrate hub")

2. Select **+**, then select **Pipeline**.

    ![The new pipeline menu option is highlighted.](../data/images/new-pipeline.png "New pipeline")
...
3. In the properties pane of the new pipeline, enter `RunUpdateCustomerDimension` in the **Name** field **(1)**, then select the **Properties** button **(2)** to hide the properties pane.
    ...
    ![The pipeline properties pane is displayed.](../data/images/pipeline-properties.png "Properties")
...
4. Under the Activities pane to the left of the design canvas, expand `Move & transform`, then drag and drop the **Data flow** activity onto the canvas.
    ...
    ![The data flow has an arrow from the activities pane to the canvas on the right.](../data/images/pipeline-add-data-flow.png "Add data flow activity")

5. Under the `General` tab, enter **UpdateCustomerDimension** for the name.

    ![The name is entered as described.](../data/images/pipeline-dataflow-general.png "General")

6. Under the `Settings` tab, select the **UpdateCustomerDimension** data flow.

    ![The settings are configured as described.](../data/images/pipeline-dataflow-settings.png "Data flow settings")

6. Select **Publish all**, then select **Publish** in the dialog that appears.

    ![The publish all button is displayed.](../data/images/publish-all-button.png "Publish all button")

7. After publishing completes, select **Add trigger** above the pipeline canvas, then select **Trigger now**.

    ![The add trigger button and trigger now menu item are both highlighted.](../data/images/pipeline-trigger.png "Pipeline trigger")

8. Navigate to the **Monitor** hub.

    ![Monitor hub.](../data/images/monitor-hub.png "Monitor hub")
...
9. Select **Pipeline runs** in the left-hand menu **(1)** and wait for the pipeline run to successfully complete **(2)**. You may have to select **Refresh (3)** several times until the pipeline run completes.
    ...
    ![The pipeline run successfully completed.](../data/images/pipeline-runs.png "Pipeline runs")
...
3. Paste the following in the query window, then select **Run** or hit F5 to execute the script and view the results:
    ...
    ![The script is displayed with the customer table output.](../data/images/first-customer-script-run.png "Customer list output")
...
2. Select **Servers** in the left-hand menu, then right-click the SQL server you added at the beginning of the lab. Select **New Query**.
    ...
    ![The New Query link is highlighted.](../data/images/ads-new-query2.png "New Query")
...
4. Select **Run** or hit `F5` to execute the query.
    ...
    ![The output is shown and the last name value is highlighted.](../data/images/customer-query-garza.png "Customer query output")
...
5. Replace **and execute** the query with the following to update the customer's last name:
    ...
    ![The customer's last name was changed to Smith.](../data/images/customer-record-updated.png "Customer record updated")
...
3. Select the **RunUpdateCustomerDimension** pipeline.
    ...
    ![The pipeline is selected.](../data/images/select-pipeline.png "Pipeline selected")
...
5. Select **OK** in the `Pipeline run` dialog to trigger the pipeline.
    ...
    ![The OK button is highlighted.](../data/images/pipeline-run.png "Pipeline run")
...
7. Select **Pipeline runs** in the left-hand menu **(1)** and wait for the pipeline run to successfully complete **(2)**. You may have to select **Refresh (3)** several times until the pipeline run completes.
    ...
    ![The pipeline run successfully completed.](../data/images/pipeline-runs2.png "Pipeline runs")
...
3. Paste the following in the query window, then select **Run** or hit F5 to execute the script and view the results:
    ...
    ![The script is displayed with the updated customer table output.](../data/images/second-customer-script-run.png "Updated customer output")
...
3. Select **SQL pools** in the left-hand menu **(1)**. Hover over the name of the dedicated SQL pool and select **Pause (2)**.
    ...
    ![The pause button is highlighted on the dedicated SQL pool.](../data/images/pause-dedicated-sql-pool.png "Pause")

4. When prompted, select **Pause**.

    ![The pause button is highlighted.](../data/images/pause-dedicated-sql-pool-confirm.png "Pause")
